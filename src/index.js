...
import { BASE_URL, getApiKey, toOpenAIImageResponse } from './helper.js';
...
async function handleImages(req, apiKey, env) {
	const { prompt } = req;
	const reqData = await transformRequest(req);
	reqData.generationConfig.responseModalities = ['TEXT', 'IMAGE'];
	reqData.contents = [
		{
			parts: [
				{
	...
	const response = await fetch(`${BASE_URL}/${API_VERSION}/models/${req.model}:generateContent?key=${apiKey}`, {
		method: 'POST',
	...
	if (ok) {
		body = await toOpenAIImageResponse(await response.json(), env);
...
const transformFnResponse = ({ content, tool_call_id }, parts) => {
	...
	const { i = 0, name = 'unknown' } = parts.calls?.[tool_call_id] ?? {};
	parts[i] = {
		functionResponse: {
